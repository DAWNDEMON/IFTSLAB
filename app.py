...
registros = forms.catcsv(elarchivo)
...
def show():
    if 'username' in session:
        cantidadregistros = 5
        listaventas = []
        listaventas = lista_clientes.ultimas_ventas(registros, cantidadregistros)
    ...
    if 'username' in session:
        ...
        if formcliente.validate_on_submit():
            ...
            if len(cliente) < 3:
                ...
                buscarclientes = lista_clientes.buscar_cliente(registros, cliente)
                if len(buscarclientes) == 0:
                    flash('No hay resultados')
                elif len(buscarclientes) == 1:
                    listar = lista_clientes.productos_cliente(registros, cliente)
...
def clientesregulares():
    if 'username' in session:
        clientes = []
        cantidad = 5
        clientes = lista_clientes.clientes_masgastaron(registros = registros, cantidad = cantidad)
    ...
    if 'username' in session:
        ...
        if formproducto.validate_on_submit():
            ...
            if len(producto) < 3:
                ...
                res_pr = lista_clientes.buscar_productos(registros, producto)
                if len(res_pr) == 0:
                    flash('No se encontraron productos')
                elif len(res_pr) == 1:
                    listar = lista_clientes.lista_producto(registros, producto)
...
def masvendidos():
    if 'username' in session:
        productos = [] 
        cantidad = 5
        productos = lista_clientes.masvendidos(registros = registros, cantidad = cantidad)
